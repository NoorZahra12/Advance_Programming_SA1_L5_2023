...
def update_order_summary(*args):
    # Update the order summary label based on user's selections
    selected_burger_type = combo_box.get()
...
# Left Canvas with Linear Background
...
# Right Canvas with Linear Background
...
combo_box = ttk.Combobox(meatframe, values=burger_meat, width=30)
combo_box.grid(row=1, column=1, padx=20)
...
for i, topping in enumerate(topping_options):
    checkbox = tk.Checkbutton(toppingframe, text=topping, variable=topping_checkbox_vars[i], onvalue=topping, offvalue="", command=on_checkbox_click)
...
for i, condiment in enumerate(condiments_option):
    checkbox = tk.Checkbutton(condimentframe, text=condiment, variable=condiment_checkbox_vars[i], onvalue=condiment, offvalue="", command=on_checkbox_click)
    checkbox.pack(anchor="w")
...
for i, side in enumerate(side_options):
    checkbox = tk.Checkbutton(sideframe, text=side, variable=side_checkbox_vars[i], onvalue=side, offvalue="", command=on_checkbox_click)
    checkbox.pack(anchor="w")

# Your Order, here a summary of user's order in real-time with "\ntotal price:" also shown at the end of the order
# Your Order Label
order_label = tk.Label(userframe, text="Your Order:", font=("Helvetica", 12, "bold"))
order_label.pack(pady=10, anchor="w")

# Label to display the order summary dynamically
order_summary_label = tk.Label(userframe, text="", font=("Helvetica", 10))
order_summary_label.pack(anchor="w")

# Label to display the total price dynamically
...
burger_prices = {"Chicken": 5.00, "Mutton": 6.00, "Beef": 7.00, "Vegetarian": 4.50}
...
combo_box.bind("<FocusOut>", update_order_summary)

for var in topping_checkbox_vars + condiment_checkbox_vars + side_checkbox_vars:
    var.trace_add("write", on_checkbox_click)
